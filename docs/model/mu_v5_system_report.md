...
## 6.6 追加検証結果（2025-12-05実施）

### 目的関数比較: LambdaRank vs Binary Classification

| 目的関数 | Valid ROI | Test ROI | Brier Score |
|----------|-----------|----------|-------------|
| **LambdaRank** | 81.64% | 78.95% | 0.0630 |
| **Binary** | 81.96% | **79.03%** | 0.0662 |

**結論**: 両者はほぼ同等（差 +0.08%）。現行のLambdaRankを維持。

### Calibration Plot（確率較正）
...
**結論**: 10-20%帯でやや過小評価傾向。中穴の予測精度改善の余地あり。

### Training-Serving Skew検証

> [!WARNING]
> `morning_odds`データは現在未取得（全てNull）のため、朝一オッズと確定オッズの乖離検証は実施できませんでした。
> 今後、出馬表スクレイピング時に`morning_odds`を取得する必要があります。

### Forward Selection結果

v5.4で追加した馬特徴量11個のForward Selection結果：

| 特徴量 | Valid ROI変化 | 結論 |
|--------|--------------|------|
| 全11特徴量 | 全てマイナス | ノイズと判定 |

**発見**: Forward SelectionではマイナスだがTest ROIでは改善。これは「正則化効果」として機能した可能性。
