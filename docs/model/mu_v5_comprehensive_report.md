...
| **v6.0 Calibration+EV** | Isotonic Regression + EV閾値フィルタ | 下記詳細参照 |

#### v6.0 Calibration + EV-Based Betting 検証結果（2025-12-05）

| 指標 | Before | After | 変化 |
|------|--------|-------|------|
| **Brier Score** | 0.0616 | 0.0602 | ✅ 2.3%改善 |
| **中穴帯(10-20%) EV平均** | 0.861 | 1.059 | ✅ +23%向上 |
| **新規投資機会（EV<1.0→≥1.0）** | - | 2,978件 | ✅ 発見 |

| EV閾値 | ROI | 投資比率 | 投資数 | 評価 |
|--------|-----|----------|--------|------|
| 従来(全投資) | 79.39% | 100% | 5,709 | - |
| 0.8 | 77.47% | 39.4% | 2,251 | ❌ ROI低下 |
| 1.0 | 71.39% | 22.2% | 1,269 | ❌ ROI低下 |
| 1.2 | 70.74% | 12.1% | 692 | ❌ ROI低下 |
| **1.5** | **81.71%** | 5.1% | 293 | ⚠️ 最高ROIだが投資数不足 |

**結論**: Calibrationにより中穴帯のEV評価は改善されたが、**EV閾値によるROI100%超えは達成不可**。投資比率5.1%（293レース）では統計的有意性が不十分であり、レビューの「EV≥1.0でROI100%超」仮説は本データでは成立しなかった。

> [!NOTE]
> **次のアプローチ候補**:
> - モデル自体の予測精度向上（新特徴量追加）
> - Top1以外の馬（Top2, Top3）への投資併用
> - 荒れるレース（高νスコア）への選択的投資

### 8.2 進行中

| タスク | 状態 | 優先度 |
|--------|------|--------|
| v5.4/v6.0をベースモデルとして確定 | ✅ 完了 | - |
| v6.0 Calibration+EV検証 | ✅ 完了 | - |
| **morning_oddsスクレイピング実装** | 🔴 未着手 | 高 |

### 8.3 予定（Phase 6 - ROI 85%超え）

- [ ] **モデル精度向上**: 馬場バイアス特徴量（開催日×コース×内外）
- [ ] タイム指数の距離補正
- [ ] σモデル（不確実性）の再学習
- [ ] νモデル（荒れ度）の再学習
- [ ] **投資戦略多様化**: Top2/Top3への複合投資
