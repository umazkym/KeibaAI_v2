...
features:
  ...
  # 有効化フラグ
  feature_recipes:
    basic_features:      { enabled: true } # 基本属性
    career_stats:        { enabled: true } # 通算成績
    past_performance:    { enabled: true } # 過去走集計
    change_flags:        { enabled: true } # 乗り替わり
    entity_stats:        { enabled: true } # 騎手・調教師・種牡馬統計
    interaction_features: { enabled: true } # 交差特徴量 (騎手×コース等)
    relative_features:   { enabled: true } # レース内相対指標 (Z-score)
  ...
  feature_selection:
    exclude_patterns:
      - '.*_name$'
      - '.*_id$'
      - '^win_odds$'            # 確定オッズは学習時除外
      - '^popularity$'          # 確定人気は学習時除外
      - '^morning_odds$'        # 一時的なオッズも除外
```
...
```yaml
models:
  # System A: μv2.6 (Odds-Weighted Binary Classsification)
  mu_model_v2_6:
    objective: "binary"
    metric: "auc"
    boosting_type: "gbdt"
    ...
    # オッズ加重学習用の重み設定はコード内で処理 (np.log1p(odds))

  # System B: μ/σ/ν (Experimental)
  lgbm_regressor:
...
```python
import pytest
from keibaai.src.parsers.common_utils import (
...
class TestPipelineE2E:
    ...
    def test_scraping_to_parsing(self, tmp_path):
        ...
        # 2. パース実行
        from keibaai.src.parsers import parse_results_html
    ...
    def test_parsing_to_features(self):
        ...
        # 2. 特徴量生成
        from keibaai.src.features import FeatureEngine
...
**場所**: `keibaai/src/validation/` + `keibaai/src/monitoring/`

KeibaAI_v2には、データ品質を自動的にチェックし、モデル性能を詳細に分析する包括的なシステムが実装されています。

### 1. ValidationPipeline（バリデーション）

**ファイル**: `keibaai/src/validation/validation_pipeline.py`
...
**ファイル**: `keibaai/src/monitoring/monitoring_local.py`
...
**ファイル**: `keibaai/src/monitoring/model_analyzer.py`
...
| `check_all_parquet.py` | パースデータ検証 | `scripts/debug/` |

**注**: 古いデバッグスクリプト（160+個）は `archive/scripts/debug/` に移動済み。
| `check_all_parquet.py` | パース済みデータの検証 |
| `check_features_data.py` | 特徴量データの検証 |
| `validate_parquet.py` | Parquetファイルの検証 |

### デバッグスクリプト例

**ファイル**: `scripts/debug/check_all_parquet.py` (現行) または `archive/scripts/debug/debug_race_results.py` (旧)
