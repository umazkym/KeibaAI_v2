# 05_パース処理詳細

**最終更新日**: 2025-12-06

---

## 📑 目次

1. [パース処理概要](#パース処理概要)
2. [4種類のパーサー](#4種類のパーサー)
3. [HTML解析パターン](#html解析パターン)
4. [共通ユーティリティ](#共通ユーティリティ)
5. [エラーハンドリング](#エラーハンドリング)
6. [データ品質改善の歴史](#データ品質改善の歴史)

---

## 🔄 パース処理概要

### 変換プロセス

```
data/raw/html/*.bin (EUC-JP encoded HTML)
    ↓
BeautifulSoup('html.parser')  # ⚠️ 'lxml'ではない
    ↓
DOM tree navigation
    ↓
pandas DataFrame
    ↓
Type conversion (Int64, float32, etc.)
    ↓
data/parsed/parquet/*.parquet
```

### パーサー一覧

| パーサー | 入力 | 出力 | サイズ |
|---------|------|------|-------|
| `results_parser.py` | `race/{race_id}.bin` | `races.parquet` | 14KB |
| `shutuba_parser.py` | `shutuba/{race_id}.bin` | `shutuba.parquet` | 14KB |
| `horse_info_parser.py` | `horse/{horse_id}_*.bin` | `horses.parquet` | 16KB |
| `pedigree_parser.py` | `ped/{horse_id}.bin` | `pedigrees.parquet` | 9KB |
| `race_detail_parser.py` | `race/{race_id}.bin` | `returns.parquet`, `race_details.parquet`, `corner_positions.parquet` | 12KB |

---

## 🏇 4種類のパーサー

### 1. results_parser.py（レース結果）

**ファイルパス**: `keibaai/src/parsers/results_parser.py`

#### メイン関数

```python
def parse_results_html(file_path: str, race_id: str = None) -> pd.DataFrame:
    """
    レース結果HTMLをパースしてDataFrameを返す

    Args:
        file_path: .binファイルパス
        race_id: レースID（Noneの場合はファイル名から抽出）

    Returns:
        pd.DataFrame: 27+列のレース結果
    """
```

#### 処理フロー

```python
# Step 1: ファイル読み込み（EUC-JPデコード）
with open(file_path, 'rb') as f:
    html_bytes = f.read()

try:
    html_text = html_bytes.decode('euc_jp', errors='replace')
except:
    html_text = html_bytes.decode('utf-8', errors='replace')

# Step 2: BeautifulSoupパース
soup = BeautifulSoup(html_text, 'html.parser')  # ⚠️ html.parserを使用

# Step 3: レースメタデータ抽出（4段階フォールバック）
race_metadata = extract_race_metadata_enhanced(soup)

# Step 4: 結果テーブル抽出
result_table = soup.find('table', class_='race_table_01')

# Step 5: 各行をパース
rows = []
for tr in result_table.find_all('tr'):
    row_data = parse_result_row_enhanced(tr, race_id, race_date, race_metadata)
    rows.append(row_data)

# Step 6: DataFrame化 + 派生特徴量
df = pd.DataFrame(rows)
df = add_derived_features(df)

# Step 7: 型変換（nullable integer）
df = convert_dtypes(df)

return df
```

#### 4段階フォールバック（距離・馬場抽出）

```python
def extract_race_metadata_enhanced(soup: BeautifulSoup) -> Dict:
    # Level 1: data_intro > diary_snap_cut > span
    race_data = soup.find('div', class_='data_intro')
    if race_data:
        snap_cut = race_data.find('div', class_='diary_snap_cut')
        if snap_cut:
            span = snap_cut.find('span')
            if span:
                text = span.get_text()
                # "ダ1200m / 天候:晴 / ダート:良 / 発走:10:10"
                match = re.search(r'(芝|ダ|障)\s*(?:右|左|直|外|内)?\s*(\d+)m', text)
                if match:
                    return extract_distance_surface(match)

    # Level 2: data_intro > span（直接）
    if race_data:
        span = race_data.find('span')
        if span:
            # ...同様の処理

    # Level 3: dl.racedata > dd
    race_data_dl = soup.find('dl', class_='racedata')
    if race_data_dl:
        dd = race_data_dl.find('dd')
        if dd:
            # ...同様の処理

    # Level 4: ページ全体からテキスト検索
    page_text = soup.get_text()
    match = re.search(r'(芝|ダ|障)\s*(\d+)m', page_text)
    if match:
        return extract_distance_surface(match)

    # すべて失敗
    logging.warning(f"Failed to extract race metadata")
    return {'distance_m': None, 'track_surface': None, ...}
```

#### trainer_id, owner_name の堅牢な抽出

```python
def parse_result_row_enhanced(tr, race_id, race_date, race_metadata):
    cells = tr.find_all('td')

    # 列数に応じた柔軟な処理
    if len(cells) >= 18:
        # 通常フォーマット
        trainer_idx = 15
        owner_idx = 16
        prize_idx = 17

        # 賞金があるか（1着のみ）
        if cells[prize_idx].get_text(strip=True).replace(',', '').isdigit():
            # 賞金列が存在
            pass
        else:
            # 賞金列がない → インデックスを調整
            owner_idx = 17

        # 調教師
        trainer_cell = cells[trainer_idx]
        trainer_link = trainer_cell.find('a')
        if trainer_link:
            trainer_id = get_id_from_href(trainer_link.get('href'), 'trainer')
            trainer_name = trainer_link.get_text(strip=True)

        # 馬主
        owner_cell = cells[owner_idx]
        owner_text = owner_cell.get_text(strip=True)
        if owner_text and owner_text not in ['---', '']:
            owner_name = normalize_owner_name(owner_text)

    return {
        'trainer_id': trainer_id,
        'trainer_name': trainer_name,
        'owner_name': owner_name,
        ...
    }
```

---

### 2. shutuba_parser.py（出馬表）

**ファイルパス**: `keibaai/src/parsers/shutuba_parser.py`

#### 主要抽出項目

```python
def parse_shutuba_html(file_path: str, race_id: str = None) -> pd.DataFrame:
    """
    出馬表HTMLをパース

    抽出項目:
    - race_id, race_date
    - bracket_number, horse_number
    - horse_id, horse_name
    - sex, age
    - basis_weight
    - jockey_id, jockey_name
    - trainer_id, trainer_name
    - horse_weight, horse_weight_change
    - morning_odds, morning_popularity
    - blinkers (ブリンカー装着フラグ)
    - prediction_mark (予想印: ◎○▲△☆)
    - scratched (取消フラグ)
    """
```

#### ブリンカー抽出

```html
<td class="HorseInfo">
  <a href="/horse/2017102294/">オメガレインボー</a>
  <span class="Blinker">B</span>  <!-- ブリンカー -->
</td>
```

```python
horse_info_cell = tr.find('td', class_='HorseInfo')
blinker_span = horse_info_cell.find('span', class_='Blinker')
blinkers = blinker_span is not None  # True or False
```

#### 予想印抽出

```html
<td>
  <span class="Icon_Mark">◎</span>  <!-- 本命印 -->
</td>
```

```python
mark_span = td_cell.find('span', class_='Icon_Mark')
prediction_mark = mark_span.get_text(strip=True) if mark_span else None
# "◎"本命 / "○"対抗 / "▲"単穴 / "△"連下 / "☆"
```

---

### 3. horse_info_parser.py（馬プロフィール・過去成績）

**ファイルパス**: `keibaai/src/parsers/horse_info_parser.py`

#### 2種類の関数

```python
def parse_horse_profile(file_path: str, horse_id: str) -> pd.DataFrame:
    """
    馬プロフィールページをパース

    抽出項目:
    - horse_id, horse_name
    - birth_date, sex, coat_color
    - trainer_id, trainer_name
    - owner_name, breeder_name, producing_area
    - sire_id, sire_name, dam_id, dam_name, damsire_id, damsire_name
    - height_cm, chest_girth_cm, cannon_bone_cm
    - sale_price
    """

def parse_horse_performance(soup: BeautifulSoup, horse_id: str) -> pd.DataFrame:
    """
    馬過去成績テーブルをパース

    抽出項目（各レースごと）:
    - horse_id, race_date, venue, weather
    - race_number, race_name, race_id
    - head_count, bracket_number, horse_number
    - finish_position, jockey_name, basis_weight
    - distance_m, track_surface
    - finish_time_seconds, margin_seconds
    - passing_order_{1,2,3,4}
    - last_3f_time, win_odds, popularity
    - horse_weight, horse_weight_change
    """
```

#### 馬体情報の抽出

```html
<table class="db_prof_table">
  <tr>
    <th>馬体高(cm)</th>
    <td>156</td>
  </tr>
  <tr>
    <th>胸囲(cm)</th>
    <td>183</td>
  </tr>
  <tr>
    <th>管囲(cm)</th>
    <td>20.5</td>
  </tr>
</table>
```

```python
prof_table = soup.find('table', class_='db_prof_table')
rows = prof_table.find_all('tr')

for row in rows:
    th = row.find('th')
    td = row.find('td')
    label = th.get_text(strip=True)
    value_text = td.get_text(strip=True)

    if '馬体高' in label:
        height_cm = int(value_text)
    elif '胸囲' in label:
        chest_girth_cm = int(value_text)
    elif '管囲' in label:
        cannon_bone_cm = float(value_text)
```

> [!NOTE]
> **データ保存状況**: 上記の馬体情報やセリ価格、および血統IDの一部は、パーサーロジック上は抽出されますが、現在のデータパイプライン設定（`horses.parquet`作成時）によっては保存されない場合があります。最新の保存状況は [03_データモデル.md](./03_データモデル.md) および [verify_parquet_schema.py](../../scripts/maintenance/verify_parquet_schema.py) の結果を参照してください。

---

### 4. pedigree_parser.py（血統）

**ファイルパス**: `keibaai/src/parsers/pedigree_parser.py`

#### 5世代血統のパース

```python
def parse_pedigree_html(file_path: str, horse_id: str) -> pd.DataFrame:
    """
    血統HTMLをパース（5世代）

    出力形式:
    | horse_id | generation | ancestor_id | ancestor_name | ancestor_birth_year | ancestor_coat_color |
    |----------|------------|-------------|---------------|---------------------|---------------------|
    | 2020104567 | 1 | 2012100123 | ディープインパクト | 2002 | 鹿毛 |
    | 2020104567 | 1 | 2010105678 | ロードクラサット | 2010 | 鹿毛 |
    | 2020104567 | 2 | 1999100001 | Sunday Silence | 1986 | 鹿毛 |
    | ... | ... | ... | ... | ... | ... |

    合計: 2 + 4 + 8 + 16 + 32 = 62行/馬
    """
```

#### HTML構造

```html
<table class="blood_table">
  <tr>
    <!-- Generation 1: 父 -->
    <td rowspan="16">
      <a href="/horse/2012100123/">ディープインパクト</a><br>
      <span class="red">2002 鹿毛</span>
    </td>

    <!-- Generation 2: 父父 -->
    <td rowspan="8">
      <a href="/horse/1999100001/">Sunday Silence</a><br>
      <span>1986 鹿毛</span>
    </td>

    <!-- Generation 3: 父父父 -->
    <td rowspan="4">
      <a href="/horse/...">Halo</a><br>
      <span>1969 黒鹿毛</span>
    </td>

    <!-- ... 以下、Generation 4, 5 と続く -->
  </tr>
</table>
```

#### 再帰的パース

```python
def parse_blood_table_recursive(soup, horse_id):
    blood_table = soup.find('table', class_='blood_table')

    pedigree_records = []

    # rowspan属性からGeneration判定
    # rowspan=16 → Gen 1
    # rowspan=8  → Gen 2
    # rowspan=4  → Gen 3
    # rowspan=2  → Gen 4
    # rowspan=1  → Gen 5

    for td in blood_table.find_all('td'):
        rowspan = int(td.get('rowspan', 1))
        generation = calculate_generation_from_rowspan(rowspan)

        link = td.find('a', href=re.compile(r'/horse/'))
        if link:
            ancestor_id = get_id_from_href(link.get('href'), 'horse')
            ancestor_name = link.get_text(strip=True)

        span = td.find('span')
        if span:
            text = span.get_text(strip=True)
            # "2002 鹿毛"
            match = re.match(r'(\d{4})\s+(\S+)', text)
            if match:
                ancestor_birth_year = int(match.group(1))
                ancestor_coat_color = match.group(2)

        pedigree_records.append({
            'horse_id': horse_id,
            'generation': generation,
            'ancestor_id': ancestor_id,
            'ancestor_name': ancestor_name,
            'ancestor_birth_year': ancestor_birth_year,
            'ancestor_coat_color': ancestor_coat_color
        })

    return pd.DataFrame(pedigree_records)
```

---

### 5. race_detail_parser.py（払い戻し・ラップ・コーナー）

**ファイルパス**: `keibaai/src/parsers/race_detail_parser.py`

#### 概要

払い戻しデータ（8種）、ラップタイム、コーナー通過順位をレースHTMLから抽出。

#### 出力ファイル

| ファイル | 件数 | 説明 |
|----------|------|------|
| `returns.parquet` | 240,333 | 払い戻し（8種×レース） |
| `race_details.parquet` | 20,157 | ラップ・コーナー生テキスト |
| `corner_positions.parquet` | 811,415 | 馬身差変換済コーナー順位 |

#### 払い戻しパース

```html
<!-- HTML構造 -->
<table summary="払い戻し">
  <tr><th>単勝</th><td>6</td><td>1,600</td><td>3</td></tr>
  <tr><th>複勝</th><td>6<br/>2</td><td>490<br/>130</td><td>4<br/>2</td></tr>
  ...
</table>
```

```python
# <br>タグで分割して複数値を抽出
def _split_by_br(td) -> List[str]:
    inner_html = str(td)
    inner_html = re.sub(r'<br\s*/?>', '|||', inner_html)
    text = re.sub(r'<[^>]+>', '', inner_html)
    return [t.strip() for t in text.split('|||') if t.strip()]
```

#### コーナー馬身差変換

```
入力: "1,3(2,5)6=4"
出力:
  horse_number=1, position=1, gap=0.0
  horse_number=3, position=2, gap=1.5
  horse_number=2, position=3, gap=2.5  ← 並走（同順位）
  horse_number=5, position=3, gap=2.5  ← 並走（同順位）
  horse_number=6, position=5, gap=3.5
  horse_number=4, position=6, gap=10.5 ← =で+7.0
```

**変換ルール**:
- `()` 内の馬: 並走（同じgap, 同じposition）
- 記号なし: +1.0馬身
- `,`: +1.5馬身（1-2馬身）
- `-`: +3.5馬身（2-5馬身）
- `=`: +7.0馬身（5馬身以上）

---

## 🧩 HTML解析パターン

### パターン1: BeautifulSoup基本

```python
soup = BeautifulSoup(html_text, 'html.parser')

# タグ検索
table = soup.find('table', class_='race_table_01')

# 複数タグ検索
rows = soup.find_all('tr')

# 属性取得
href = link.get('href')
rowspan = td.get('rowspan', 1)  # デフォルト値指定

# テキスト取得
text = td.get_text(strip=True)  # 前後の空白削除
```

### パターン2: 正規表現

```python
import re

# URL からID抽出
match = re.search(r'/horse/(\w+)/', href)
horse_id = match.group(1) if match else None

# テキストから数値抽出
match = re.search(r'(\d+)m', text)
distance_m = int(match.group(1)) if match else None

# 複雑なパターン
match = re.search(r'(\d+)回(\S+?)(\d+)日目', text)
if match:
    round_of_year = int(match.group(1))
    venue = match.group(2)
    day_of_meeting = int(match.group(3))
```

### パターン3: フォールバック

```python
# 複数の抽出方法を試す
value = None

# 方法1
elem = soup.find('div', class_='primary')
if elem:
    value = elem.get_text(strip=True)

# 方法2（フォールバック）
if not value:
    elem = soup.find('span', class_='secondary')
    if elem:
        value = elem.get_text(strip=True)

# 方法3（最終フォールバック）
if not value:
    elem = soup.find('p')
    if elem:
        value = elem.get_text(strip=True)
```

---

## 🛠️ 共通ユーティリティ

**ファイル**: `keibaai/src/parsers/common_utils.py`

### parse_time_to_seconds()

```python
def parse_time_to_seconds(time_str: str) -> Optional[float]:
    """
    タイム文字列を秒数に変換

    Examples:
        "1:23.4" → 83.4
        "59.8" → 59.8
        "2:00.0" → 120.0
    """
    if not time_str or time_str.strip() in ['', '-']:
        return None

    try:
        # "1:23.4" 形式
        if ':' in time_str:
            parts = time_str.split(':')
            minutes = int(parts[0])
            seconds = float(parts[1])
            return minutes * 60 + seconds
        # "59.8" 形式
        else:
            return float(time_str)
    except ValueError:
        return None
```

### parse_margin_to_seconds()

```python
def parse_margin_to_seconds(margin_str: str) -> Optional[float]:
    """
    着差文字列を秒数に変換

    Examples:
        "3" → 0.6秒（3馬身 = 3 * 0.2）
        "1 1/2" → 0.3秒（1.5馬身）
        "クビ" → 0.02秒
        "ハナ" → 0.01秒
        "アタマ" → 0.05秒
    """
    if not margin_str or margin_str.strip() in ['', '-']:
        return None

    # 特殊ケース
    special_cases = {
        'ハナ': 0.01,
        'クビ': 0.02,
        'アタマ': 0.05,
        '同着': 0.0
    }

    for key, value in special_cases.items():
        if key in margin_str:
            return value

    # 馬身換算（1馬身 = 0.2秒）
    try:
        # "3" → 3.0
        if margin_str.isdigit():
            return float(margin_str) * 0.2

        # "1 1/2" → 1.5 → 0.3
        if ' ' in margin_str:
            parts = margin_str.split()
            integer_part = int(parts[0])
            fraction_part = eval(parts[1])  # "1/2" → 0.5
            total_bodies = integer_part + fraction_part
            return total_bodies * 0.2

        return None

    except:
        return None
```

### parse_sex_age()

```python
def parse_sex_age(sex_age_str: str) -> Tuple[Optional[str], Optional[int]]:
    """
    性齢文字列を分解

    Examples:
        "牡3" → ("牡", 3)
        "牝4" → ("牝", 4)
        "セ6" → ("セ", 6)
    """
    if not sex_age_str:
        return (None, None)

    sex = sex_age_str[0] if len(sex_age_str) >= 1 else None
    age_str = sex_age_str[1:] if len(sex_age_str) >= 2 else None

    try:
        age = int(age_str)
    except:
        age = None

    return (sex, age)
```

### parse_horse_weight()

```python
def parse_horse_weight(weight_str: str) -> Tuple[Optional[int], Optional[int]]:
    """
    馬体重文字列を分解

    Examples:
        "476(+2)" → (476, +2)
        "480(-4)" → (480, -4)
        "計不" → (None, None)
        "除外" → (None, None)
    """
    if not weight_str or weight_str in ['計不', '除外', '-']:
        return (None, None)

    # "476(+2)" をパース
    match = re.match(r'(\d+)\(([\+\-]?\d+)\)', weight_str)
    if match:
        weight = int(match.group(1))
        change = int(match.group(2))
        return (weight, change)

    # "476" のみ（増減なし）
    match = re.match(r'(\d+)', weight_str)
    if match:
        weight = int(match.group(1))
        return (weight, 0)

    return (None, None)
```

---

## 🚨 エラーハンドリング

### SQLiteメタデータ管理

```python
import sqlite3

def save_parse_error(file_path: str, error_message: str, conn: sqlite3.Connection):
    """パースエラーをSQLiteに記録"""
    cursor = conn.cursor()
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS parse_errors (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            file_path TEXT NOT NULL,
            error_message TEXT,
            timestamp DATETIME DEFAULT CURRENT_TIMESTAMP
        )
    """)

    cursor.execute("""
        INSERT INTO parse_errors (file_path, error_message)
        VALUES (?, ?)
    """, (file_path, error_message))

    conn.commit()
```

### パース失敗時のログ

```python
try:
    df = parse_results_html(file_path)
except Exception as e:
    logging.error(f"Parse failed: {file_path}, {e}", exc_info=True)
    save_parse_error(file_path, str(e), conn)
    return pd.DataFrame()  # 空のDataFrameを返す
```

---

## 📈 データ品質改善の歴史

### v1.0 → v1.5（2023-10）

#### 改善内容

1. **HTMLパーサーの変更**
   - `lxml` → `html.parser`
   - 理由: lxmlのバージョン依存性、互換性問題

2. **レース結果の欠損データ削減**
   - Before: 10.2% missing
   - After: 0.0% missing
   - 手法: 4段階フォールバック

3. **新規カラム追加（11個）**
   - race_name, distance_m, track_surface, weather, track_condition
   - venue, day_of_meeting, round_of_year
   - race_class, age_restriction, head_count

4. **trainer_id, owner_name の抽出改善**
   - Before: 30% missing
   - After: 5% missing
   - 手法: 列数に応じた柔軟なインデックス調整

### v1.5 → v2.0（2025-11）

#### 改善内容

1. **派生特徴量の追加**
   - pace_index, position_change_*, popularity_finish_diff
   - time_except_last3f, final_corner_to_finish

2. **horses_performance データの保存**
   - Before: パース処理のみ、保存なし
   - After: horses_performance.parquet として保存

3. **血統データの拡張**
   - ancestor_birth_year, ancestor_coat_color 追加

---

**次のドキュメント**: [06_特徴量エンジニアリング.md](./06_特徴量エンジニアリング.md)
