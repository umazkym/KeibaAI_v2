...
> [!NOTE]
> **実装状況 (2025-12)**: 本ロードマップの Phase 1 は完了し、**System A (V15 + V4.4 Residual Ensemble)** として実装完了しました。詳細は [07_機械学習モデル.md](./07_機械学習モデル.md) を参照してください。
>
> **μモデル汎用化検証 (2025-12-26)**: μモデル予測をV15に統合する2つのアプローチを検証したが、ROI改善には至らず。精度向上（Spearman相関）がROI改善に直結しない教訓を得た。
...
## 10. V17改善計画（2025-12-24追記）

### 10.1 深層分析から発見された弱点

| 弱点 | 内容 | 影響 |
|------|------|------|
| 夏場（8-9月） | ROI 60-70%と低下 | 年間平均を低下 |
| 苦手競馬場 | 札幌(64%)、新潟(69%)、阪神(69%) | 特定場所で損失 |
| V4.4残差計算 | 「過小評価」の定義が曖昧 | 改善余地あり |

### 10.2 V17で検証予定の改善

| # | 改善点 | 実装方針 | リスク |
|---|--------|----------|--------|
| 1 | 夏場対策 | 季節sin/cos + 馬の夏場過去勝率 | 低 |
| 2 | 苦手競馬場 | 馬の競馬場別勝率（shift(1)） | 中 |
| 3 | V4.4残差改良 | 残差標準化 or ランク差ベース | 低 |
| 4 | 特徴量追加 | ラップタイム、血統×距離適性 | 中 |

> [!CAUTION]
> 全ての改善はshift(1)厳守、4年間Walk-forward検証、Gap 30%超は却下。

### 10.3 期待効果

- 夏場ROI: 60% → 80%（+20%改善）
- 苦手競馬場: 65% → 80%（+15%改善）
- 全体ROI: 83.9% → 85-87%（+1-3%改善）

---

## 11. 6年間Walk-forward検証結果（2026-01-04追記）

2020-2025年の6年間（約28万レース）を対象にWalk-forward検証を実施。詳細は [26_6年Walk-forward検証サマリー.md](./26_6年Walk-forward検証サマリー.md) を参照。

**主要な発見**:
- 競馬場間でROI差18%（小倉87.6% vs 福島69.7%）を確認
- レースの「荒れ度」を1番人気オッズで判定可能
- 馬単2-3倍帯でROI 83.7%（最高効率）を達成
- 穴馬発見時の平均払戻: 18.6倍

---

## 12. 次期開発ロードマップ（2026-01〜）

### 12.1 Phase 1: モデルチューニング（2週間）

| タスク | 説明 | 期待効果 |
|:-------|:-----|:---------|
| **競馬場×芝ダート×距離帯別チューニング** | 苦手条件の特定と対策 | 苦手競馬場ROI +5% |
| 得意条件フィルタリング | ROI 80%未満条件は賭け金50%減 | リスク軽減 |
| 条件別ROIテーブル作成 | 自動判定の基礎データ | 戦略自動化の基盤 |

### 12.2 Phase 2: 荒れ度フィルター実装（1週間）

```python
# 1番人気オッズによる自動判定ロジック
if fav1_odds <= 1.5:
    return "stable"      # → 馬単/三連単で2-3着穴馬狙い
elif fav1_odds <= 3.0:
    return "moderate"    # → 穴馬単勝狙い（10-20倍帯）
else:
    return "volatile"    # → 複勝でリスクヘッジ
```

### 12.3 Phase 3: 2-3着予測精度向上（3週間）

| タスク | 説明 | 期待効果 |
|:-------|:-----|:---------|
| LambdaRankモデルで順位全体を学習 | 1着だけでなく2-3着も最適化 | 馬単ROI向上 |
| 2着・3着専用特徴量開発 | `horse_2nd_rate`, `horse_close_finish_rate` | 精度+5% |
| 馬単専用モデル（Model B）構築 | 単勝モデルとは別に最適化 | 馬単ROI 85%目標 |

### 12.4 Phase 4: 自動戦略切り替えシステム（2週間）
...
実装ファイル:
- scripts/betting/race_type_classifier.py
- scripts/betting/bet_strategy_selector.py
- scripts/betting/auto_betting_orchestrator.py
```
...
| レースタイプ | 判定条件 | 推奨券種 | 根拠 |
|:-------------|:---------|:---------|:-----|
| **安定レース** | 1番人気≤1.5倍 | 馬単/三連単 | 2-3着に穴馬25%出現 |
| **中程度** | 1番人気2-3倍 | 単勝10-20倍帯 | ROI 80.1%で安定 |
| **荒れやすい** | 1番人気≥3倍 | 複勝 or 見送り | 的中率低下、リスク回避 |

---

## 13. V16 jw_rank_in_race 検証結果（2026-01-16更新）

### 検証内容
- **jw_rank_in_race**: 騎手勝率のレース内相対順位（0-1正規化）
- **team_power**: 騎手勝率 + 調教師勝率の平均
...
## 14. 特徴量エンジニアリング検証結果（2026-01-16）

### 検証した新特徴量

| 特徴量 | 定義 | 効果 | 結論 |
|--------|------|------|------|
| is_comeback_candidate | 前走7-10着 & 200m以上短縮 | +0.0% | ❌ 既存特徴量で代替可能 |
| prev_time_avg（タイム指数） | 標準化上がり3F偏差値 | +1.5% | △ 限定的（V15にhorse_last3f_avg既存） |
| jockey_change | 騎手乗り替わりフラグ | -2.1% | ❌ 不安定 |
...
| 馬場別（芝/ダート）分割 | -0.7% | ❌ |
| 距離別（4分割）分割 | -1.7% | ❌ |
| 具体的コース別（127コース） | 一部で+20%超の改善 | △ 要追加検証 |

### 有望コース（8年Walk-forward検証）

| コース | 平均改善 | 標準偏差 | 過適合チェック |
|--------|---------|---------|--------------|
| 京都_1400m_ダート | +28.6% | ±51.5% | 後半で大幅改善（過適合なし） |
| 小倉_1700m_ダート | +5.0% | ±15.9% | 安定だが大きな効果なし |
...
> **市場効率性の再確認**
> - 公開データからの追加価値発見は困難
> - V15+V4.4（ROI 77-82%）が現時点の最適解
> - 大幅改善には非公開データ（調教、リアルタイムオッズ変動等）が必要

**検証スクリプト**: `scripts/temp/thorough_course_validation.py`
