...
### 問題
```powershell
cd keibaai && python ../scripts/training/predict.py --date 2023-12-31
```
エラー:
```
...
現在、以下のデータ項目はソースHTMLまたはパース処理の制約により、Parquetファイルに含まれていません。特徴量エンジニアリングや分析の際はご注意ください。

| 対象テーブル | 欠損項目 | 備考 |
|------------|---------|------|
| `races` | `head_count` | 出走頭数は `shutuba` テーブルのレコード数から `groupby` 等で動的に算出する必要があります。 |
| `shutuba` | `blinkers` | ブリンカー装着情報は現在取得されていません。 |
| `shutuba` | `prediction_mark` | 予想印は現在取得されていません。 |
| `horses` | `coat_color`, `sire_id` 等 | 詳細なプロフィール・血統情報は `pedigrees` テーブルのみに含まれる場合があります。 |
| `pedigrees`| `ancestor_birth_year`, `coat_color` | 先祖馬の生年・毛色は現在取得されていません。 |
