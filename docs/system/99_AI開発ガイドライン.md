...
*   **ドキュメント**: プロジェクトに関する**注**: 本ドキュメントの指示は、`CLAUDE.md` や `docs/system/` 以下の公式ドキュメントに従うものとします。ファイルやディレクトリ構成はプロジェクトの進化に伴い変更される可能性があります。特に、本ガイドライン内で例示されているファイルパス（`scripts/xxx.py` 等）は説明用のものであり、必ずしも実在するとは限りません。
*   **思考プロセス**:
...
> [!IMPORTANT]
> モデル構築ごとに専用のバージョンフォルダを作成し、全ての関連ファイルを格納してください。

### 必須ルール

1. **フォルダ構造**: `keibaai/models/{version_name}/` に全ファイルを格納
2. **必須ファイル**:
...
```
keibaai/models/v26_restored/
├── config.yaml           # ハイパーパラメータ
├── report.md             # ROI結果サマリー
├── features.txt          # 66特徴量リスト
├── multi_period_results.csv
└── development_log.md    # 失敗仮説・教訓
```

### ベストプラクティス

- **不変性**: 作成済みバージョンは変更しない。改善は新バージョンで
- **失敗記録**: 何がうまくいかなかったかを `development_log.md` に記録
- **比較可能性**: 同一条件（テスト期間、データ範囲）で評価

詳細は [15_モデル命名規則とベストプラクティス.md](./15_モデル命名規則とベストプラクティス.md) を参照。
