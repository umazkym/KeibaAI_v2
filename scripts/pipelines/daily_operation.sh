...
# プロジェクトのルートディレクトリ (このスクリプトの親の親の親)
...
$PYTHON_EXEC scripts/pipelines/run_scraping_pipeline_local.py
...
# $PYTHON_EXEC scripts/pipelines/parse_all.py --date ${TARGET_DATE}
# (アップロードされた run_parsing_pipeline_local.py を使う場合)
$PYTHON_EXEC scripts/pipelines/run_parsing_pipeline_local.py

echo "[3/11] 特徴量生成中..."
# (仕様書 17.2 の generate_features.py を使用)
$PYTHON_EXEC scripts/pipelines/generate_features.py \
    --date ${TARGET_DATE} \
...
$PYTHON_EXEC scripts/training/predict.py \
    --date ${TARGET_DATE} \
...
$PYTHON_EXEC scripts/simulation/simulate_daily_races.py \
    --date ${TARGET_DATE} \
    --K 1000 \
    --model_id "latest" \
    --config "configs/default.yaml"
...
$PYTHON_EXEC scripts/optimization/optimize_daily_races.py \
    --date ${TARGET_DATE} \
    --W_0 100000 \
    --config "configs/default.yaml" \
