...
def main():
    ...
    # --- 3.5. 確率較正 (Isotonic Regression) ---
    logging.info("=" * 60)
    logging.info("確率較正 (Isotonic Regression) を実行中...")
    logging.info("=" * 60)
    ...
    try:
        ...
        if 'race_date' in final_df.columns:
            ...
            # race_dateがない場合は単純に分割
            split_idx = int(len(final_df) * 0.8)
            train_df_calib = final_df.iloc[:split_idx]
            val_df_calib = final_df.iloc[split_idx:]
            logging.warning("race_dateがないため、ランダムに分割しました。")
        
        # 検証データで予測を実行（未較正の予測確率を取得）
        X_val = val_df_calib[feature_names]
        y_val_is_win = (val_df_calib['finish_position'] == 1).astype(int).values
        
        # rankerから予測スコアを取得
        y_val_pred_score = estimator.model_ranker.predict(X_val)
        
        # ★重要な修正: Sigmoid変換を削除★
        # Sigmoid変換は入力範囲を0.74-0.81に圧縮し、Isotonic Regressionの学習を妨げていた
        # Rankerスコアを直接使用することで、適切な較正曲線を学習可能にする
        y_val_pred_proba = y_val_pred_score
        
        calibrator = IsotonicCalibrator(increasing='auto')
        calibrator.fit(
            y_true=y_val_is_win,
            y_pred_proba=y_val_pred_proba
        )
        
        logging.info("✓ 確率較正が完了しました")
        
    except Exception as e:
        logging.error(f"確率較正中にエラーが発生しました: {e}", exc_info=True)
        logging.warning("較正器なしでモデルを保存します。")
        calibrator = None
    ...
    try:
        ...
        if calibrator is not None:
            calibrator_path = output_path / 'calibrator.pkl'
            calibrator.save(calibrator_path)
            logging.info(f"  - calibrator.pkl (Isotonic Regression)")
        
        # モデル情報を保存
        import json
        model_info = {
            'version': 'mu_phase_f_corrected',
            'created_at': datetime.now().isoformat(),
            'train_period': f"{args.start_date} to {args.end_date}",
            'num_features': len(estimator.feature_names),
            'calibrated': calibrator is not None,
            'brier_score_before': calibrator.brier_score_before if calibrator else None,
            'brier_score_after': calibrator.brier_score_after if calibrator else None,
            'sigmoid_removed': True  # 修正版フラグ
        }
        
        model_info_path = output_path / 'model_info.json'
        ...
        if calibrator is not None:
            logging.info(f"  - calibrator.pkl")
        logging.info(f"  - model_info.json")
