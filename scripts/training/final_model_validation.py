...
V15+V4.4アンサンブルを4年間検証
60倍以上除外時のROI、オッズ帯別安定性も確認
"""
import pandas as pd
import numpy as np
...
def calc_roi_detailed(df, preds, max_odds_list=[999, 60, 40, 20]):
    """ROI詳細計算（オッズ帯別、高オッズ除外も含む）"""
    d = df.copy()
    d['score'] = preds
    ...
    results['全体'] = {
        'ベット': n_bets,
        '的中': len(hits),
        '的中率': len(hits) / n_bets * 100 if n_bets > 0 else 0,
        'ROI': hits['win_odds'].sum() / n_bets * 100 if n_bets > 0 else 0
    }
    
    # 高オッズ除外時のROI
    for max_odds in max_odds_list[1:]:
        hits_filtered = hits[hits['win_odds'] <= max_odds]
        results[f'{max_odds}倍以下'] = {
            'ROI': hits_filtered['win_odds'].sum() / n_bets * 100 if n_bets > 0 else 0
        }
    
    # オッズ帯別ROI
    odds_bands = [(1, 5), (5, 10), (10, 20), (20, 40), (40, 60), (60, 999)]
    for low, high in odds_bands:
        ...
        if len(band_bets) > 0:
            results[f'{low}-{high}倍'] = {
                'ベット': len(band_bets),
                '的中': len(band_hits),
                '的中率': len(band_hits) / len(band_bets) * 100,
...
def main():
    ...
    print("=" * 80)
    print("最終モデル検証 - V15+V4.4アンサンブル（ROI確からしさ含む）")
    print("=" * 80)
    ...
    for year, train_end, valid_start, valid_end, test_start, test_end in test_periods:
        ...
        # 詳細ROI計算
        roi_detail = calc_roi_detailed(test_f, preds_ensemble)
        
        print(f"\n  【単勝ROI】")
        print(f"    全体:      {roi_detail['全体']['ROI']:.1f}% (的中率 {roi_detail['全体']['的中率']:.1f}%)")
        print(f"    60倍以下:  {roi_detail.get('60倍以下', {}).get('ROI', 0):.1f}%")
        print(f"    40倍以下:  {roi_detail.get('40倍以下', {}).get('ROI', 0):.1f}%")
        
        print(f"\n  【オッズ帯別ROI】")
        for key in ['1-5倍', '5-10倍', '10-20倍', '20-40倍', '40-60倍', '60-999倍']:
            if key in roi_detail:
                d = roi_detail[key]
                print(f"    {key}: ベット{d['ベット']:,}件, 的中{d['的中']:,}件, ROI {d['ROI']:.1f}%")
        
        all_results.append({
            'year': year,
            'roi_all': roi_detail['全体']['ROI'],
            'roi_60': roi_detail.get('60倍以下', {}).get('ROI', 0),
            'roi_40': roi_detail.get('40倍以下', {}).get('ROI', 0),
            'hit_rate': roi_detail['全体']['的中率'],
        })
    ...
    print(f"\n{'年':>6} {'全体ROI':>10} {'60倍以下':>10} {'40倍以下':>10} {'的中率':>8}")
    print("-" * 50)
    
    for r in all_results:
        print(f"{r['year']:>6} {r['roi_all']:>9.1f}% {r['roi_60']:>9.1f}% {r['roi_40']:>9.1f}% {r['hit_rate']:>7.1f}%")
    
    avg_all = np.mean([r['roi_all'] for r in all_results])
    avg_60 = np.mean([r['roi_60'] for r in all_results])
    avg_40 = np.mean([r['roi_40'] for r in all_results])
    avg_hit = np.mean([r['hit_rate'] for r in all_results])
    
    print("-" * 50)
    print(f"{'平均':>6} {avg_all:>9.1f}% {avg_60:>9.1f}% {avg_40:>9.1f}% {avg_hit:>7.1f}%")
    
    print("\n" + "=" * 80)
    print("ROI確からしさ評価")
    print("=" * 80)
    
    high_odds_impact = avg_all - avg_60
    print(f"\n  全体ROI: {avg_all:.1f}%")
    print(f"  60倍以下ROI: {avg_60:.1f}%")
    print(f"  高オッズ（60倍超）の貢献: {high_odds_impact:.1f}%")
    
    if high_odds_impact < 5:
        print(f"\n  ✅ 再現性あり: 高オッズ依存は{high_odds_impact:.1f}%と少ない")
    else:
        print(f"\n  ⚠️ 注意: 高オッズ依存が{high_odds_impact:.1f}%と大きい")
