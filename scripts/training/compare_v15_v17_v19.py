...
from keibaai.src.features.leak_free_feature_engineer_v19 import LeakFreeFeatureEngineerV19
...
def main():
    logger.info("=" * 60)
    logger.info("V15 vs V17 vs V19 比較検証（ドキュメント同一条件）")
    logger.info("=" * 60)
    ...
    result_v17 = train_and_evaluate(engine_v17, train, test, "V17", params)
    results.append(result_v17)
    logger.info(f"V17: Train ROI={result_v17['train_roi']:.1f}%, Test ROI={result_v17['test_roi']:.1f}%, Gap={result_v17['gap']:.1f}%")
    
    # V19
    logger.info("\n--- V19 ---")
    engine_v19 = LeakFreeFeatureEngineerV19()
    engine_v19.fit(train, pedigrees, corners, race_details, None, horses)
    result_v19 = train_and_evaluate(engine_v19, train, test, "V19", params)
    results.append(result_v19)
    logger.info(f"V19: Train ROI={result_v19['train_roi']:.1f}%, Test ROI={result_v19['test_roi']:.1f}%, Gap={result_v19['gap']:.1f}%")
    ...
    # V15 vs V17
    logger.info(f"\n--- V17 vs V15 ---")
    logger.info(f"Test ROI: {result_v17['test_roi'] - result_v15['test_roi']:+.1f}%")
    logger.info(f"Gap:      {result_v17['gap'] - result_v15['gap']:+.1f}%")
    logger.info(f"Features: {result_v17['n_features'] - result_v15['n_features']:+d}")
    
    # V19 vs V15
    logger.info(f"\n--- V19 vs V15 ---")
    logger.info(f"Test ROI: {result_v19['test_roi'] - result_v15['test_roi']:+.1f}%")
    logger.info(f"Gap:      {result_v19['gap'] - result_v15['gap']:+.1f}%")
    logger.info(f"Features: {result_v19['n_features'] - result_v15['n_features']:+d}")
