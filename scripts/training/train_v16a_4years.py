...
def main():
    ...
    for year, train_end, valid_start, valid_end, test_start, test_end in test_periods:
        ...
        model = train_model(train_f, valid_f, all_features, params)
    ...
    # サマリー
    print("\n" + "=" * 80)
    print("V16a（障害除外のみ）検証結果サマリー")
    print("=" * 80)
    
    print(f"\n{'年':>6} {'Test ROI':>12} {'Gap':>10}")
    print("-" * 40)
    
    for r in all_results:
        print(f"{r['year']:>6} {r['test_roi']:>11.1f}% {r['gap']:>9.1f}%")
    
    avg_test = np.mean([r['test_roi'] for r in all_results])
    avg_gap = np.mean([r['gap'] for r in all_results])
    
    print("-" * 40)
    print(f"{'平均':>6} {avg_test:>11.1f}% {avg_gap:>9.1f}%")
    
    print("\n" + "=" * 80)
    print("V15 vs V16a 比較")
    print("=" * 80)
    print(f"\n  V15 (従来):      76.1%")
    print(f"  V16a (障害除外): {avg_test:.1f}%")
