#!/usr/bin/env python3
"""
μモデル v5.4 学習スクリプト（クリーン評価版）

【重要修正】
- OptunaでTestを一切参照しない（Test Leakage除去）
- Valid ROIのみで最適化
- Testは最終確認でのみ1回だけ評価

【目標】真のv3.5超え（Test ROI > 81.84%）
"""
...
class MuV54Trainer:
    ...
    def __init__(self):
        ...
        self.output_dir = Path('keibaai/models/mu_v5_4')
    ...
    def get_distance_category(distance_m):
        for cat, (low, high) in MuV54Trainer.DISTANCE_CATEGORIES.items():
    ...
    def generate_horse_features(self, df, races_df):
        """馬特徴量生成"""
        logging.info("=" * 60)
        logging.info("v5.4: 馬特徴量生成（クリーン評価版）")
        logging.info("=" * 60)
    ...
    def train(self, df, feature_cols, n_trials=50):
        logging.info("=" * 60)
        logging.info("μモデル v5.4 学習開始（クリーン評価版）")
        ...
        def objective(trial):
            ...
            # ★★★ Validのみで評価（Testを一切参照しない！） ★★★
            valid_roi = calculate_roi(valid_df, model.predict(valid_df[feature_cols]))
            
            return valid_roi  # Pure Valid optimization
        
        logging.info(f"Optuna {n_trials}トライアル（Valid専用最適化）...")
        ...
        logging.info(f"最良のパラメータ: {best_params}")
        ...
        logging.info("=" * 60)
        logging.info("【最終結果: μ v5.4（クリーン評価版）】")
        logging.info("=" * 60)
        logging.info(f"  Valid ROI: {valid_roi:.2%}")
        logging.info(f"  Test ROI: {test_roi:.2%} ← 最終確認（初見）")
    ...
    def save_model(self, features, valid_roi, test_roi, best_params):
        with open(self.output_dir / 'mu_v5_4_ranker.pkl', 'wb') as f:
        ...
        metadata = {
            'version': 'v5.4', 'description': 'クリーン評価版（Valid専用最適化）',
