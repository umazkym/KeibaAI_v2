"""
V10.2 モデル訓練・ROI検証スクリプト
- V10 vs V10.1 vs V10.2 の比較
- jt_combo削除による過学習抑制効果を確認
"""
import pandas as pd
import numpy as np
import sys
...
print("=" * 80)
print("V10.2 モデル訓練・ROI検証（jt_combo削除）")
print("=" * 80)
...
# V10.2 新特徴量 + jt_combo削除
print("\n4. V10.2 (新特徴量 - jt_combo) 訓練...")
v10_2 = LeakFreeFeatureEngineerV10_2()
v10_2_results = train_and_evaluate(v10_2, train, test, "V10.2", corners_df=corners, horses_df=horses, pedigrees_df=pedigrees)
...
results_df = pd.DataFrame([v10_1_results, v10_2_results])
print(results_df.to_string(index=False))

# 改善度
roi_improvement = v10_2_results['test_roi'] - v10_1_results['test_roi']
gap_improvement = v10_1_results['gap'] - v10_2_results['gap']
print(f"\n【V10.2 改善効果】")
print(f"  ROI: {v10_1_results['test_roi']:.1f}% → {v10_2_results['test_roi']:.1f}% ({roi_improvement:+.1f}%)")
print(f"  過学習Gap: {v10_1_results['gap']:.1f}% → {v10_2_results['gap']:.1f}% ({gap_improvement:+.1f}%)")

if roi_improvement > 0 and gap_improvement > 0:
    print("✅ ROI改善＆過学習抑制に成功")
