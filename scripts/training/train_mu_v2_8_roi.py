#!/usr/bin/env python3
"""
μv2.8 モデル学習スクリプト (train_mu_v2_8_roi.py)

【変更点 from v2.7】
- ROI最大化戦略 Phase 4: レポートメトリクスの導入 (Report Metrics Integration)
- 新特徴量:
    - time_deviation_score_avg_5: 基準タイムからの偏差（過去5走平均）
    - l3f_deviation_score_avg_5: 基準上がり3Fからの偏差（過去5走平均）
    - fast_track_score_avg: 高速馬場での偏差平均
    - heavy_track_score_avg: タフ馬場での偏差平均
- オッズ加重学習 (Odds-Weighted Training) は継続。
- 保存先ディレクトリを `keibaai/models/mu_v2_8` に変更。
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('train_mu_v2_8.log')
    ]
)

class MuV28ROITrainer:
    def __init__(self, data_dir: str = 'keibaai/data/parsed/parquet'):
        self.data_dir = Path(data_dir)
        # v2.8用のディレクトリ
        self.models_dir = Path('keibaai/models/mu_v2_8')
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        if not perf_path.exists():
            # Fallback to normal file if fixed doesn't exist
        ...
        if races_path.exists():
            ...
            cols_to_add = ['round_of_year', 'day_of_meeting', 'track_condition', 'venue', 'track_surface', 'win_odds', 'race_class', 'race_name', 'popularity']
        ...
        # ★ 重要: 血統ID (sire_id, damsire_id) をpedigrees.parquetから抽出してdfにマージ
        if not ped_df.empty:
            try:
    ...
    def generate_rolling_features(self, df: pd.DataFrame, ped_df: pd.DataFrame, prof_df: pd.DataFrame, dry_run: bool = False) -> pd.DataFrame:
        ...
        """
        logging.info("=" * 50)
        logging.info("ローリング特徴量生成を開始します (v2.8 Report Metrics Mode)")
        logging.info("=" * 50)
        ...
        # Dry Run時は検証のため2022年から開始
        if dry_run:
            training_start_date = pd.Timestamp('2022-01-01')
            logging.info("Dry Run Mode: 開始日を 2022-01-01 に設定します")
        ...
        # 保存 (v2.8用のファイル名)
        if not dry_run:
            save_path = self.models_dir / 'train_data_mu_v2_8.parquet'
    ...
    def train_model(self, df: pd.DataFrame, n_trials: int = 50):
        ...
        logging.info("=" * 50)
        logging.info("モデル学習を開始します (v2.8 Weighted Training)")
        logging.info("=" * 50)
        ...
        # モデル保存
        model_path = self.models_dir / 'mu_v2_8_model.txt'
        model.save_model(str(model_path))
        
        with open(self.models_dir / 'mu_v2_8_model.pkl', 'wb') as f:
            pickle.dump(model, f)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Train μ v2.8 model with Report Metrics')
    ...
    trainer = MuV28ROITrainer()
    
    # 1. データ読み込み
    df, ped_df, prof_df = trainer.load_data()
    
    # 2. 特徴量生成
    # v2.8用のデータファイルが存在するか確認
    train_data_path = trainer.models_dir / 'train_data_mu_v2_8.parquet'
