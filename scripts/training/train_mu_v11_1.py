#!/usr/bin/env python3
"""
μモデル v11.1 学習スクリプト（穴馬発掘強化版 + 強正則化）
...
class MuV111Trainer:
    """μモデル v11.1 穴馬発掘強化版 + 強正則化"""
    
    # 過学習防止のための正則化パラメータ範囲
    # 強化された正則化パラメータ（過学習抑制）
    REGULARIZATION_RANGES = {
        'num_leaves': (15, 40),  # 小さめに
        'lambda_l1': (10.0, 50.0),  # 高めに
        'lambda_l2': (8.0, 30.0),  # 高めに
        'min_child_samples': (120, 300),  # 高めに
        'learning_rate': (0.008, 0.03),  # 低めに
        'feature_fraction': (0.35, 0.55),  # 低めに
        'bagging_fraction': (0.55, 0.75),
        'max_depth': (3, 5),  # 浅めに
    }
    ...
    def __init__(self, train_cutoff: str = '2023-01-01'):
        ...
        self.output_dir = Path('keibaai/models/mu_v11_1')
    ...
    def save_model(self, features, valid_roi, test_roi, gap, best_params, 
                   valid_fav1_rate, test_fav1_rate):
        """モデル保存"""
        with open(self.output_dir / 'mu_v11_1_ranker.pkl', 'wb') as f:
