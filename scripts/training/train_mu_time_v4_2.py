...
log_file = log_dir / f"train_mu_time_v4_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def train_model(df, feature_cols, train_start, train_end, test_start, test_end):
    ...
    params = {
        ...
        'reg_alpha': 1.0,  # V4: 2.0 → V4.1: 1.0
        'reg_lambda': 1.5,  # V4: 3.0 → V4.1: 1.5
...
def save_model(model, feature_cols, metrics, base_time_stats):
    """モデル保存"""
    output_dir = project_root / "keibaai" / "models" / "mu_time_v4_2"
    ...
    config = {
        'version': 'mu_time_v4.2',
        'description': '展開予測特徴量追加版（脱質・ペース予測）',
...
def main():
    logger.info("=" * 60)
    logger.info("μ_time_v4.2 展開予測特徴量追加版 訓練スクリプト開始")
    logger.info("=" * 60)
