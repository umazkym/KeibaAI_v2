...
【v3.6の反省】
オッズ重みを強くしすぎると高配当狙いに偏りROI低下

【v3.7のアプローチ】
1. v3.5のGap Feature拡張を維持
2. オッズ重みは適度（log1p）
3. より多くのOptunaトライアルで最適化
4. 外れ値処理の強化
...
class MuV37Trainer:
    ...
    def add_extended_gap_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col, name in [
            ...
            if col in df.columns:
                ascending = col in ['combo_avg_finish', 'past_5_finish_position_mean']
                df[f'{name}_rank'] = df.groupby('race_id')[col].rank(ascending=not ascending, method='first')
    ...
    def add_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """交互作用特徴量を追加"""
        logging.info("交互作用特徴量を追加中...")
        
        # 【新規】Gap × 安定性
        if 'gap_ability_popularity' in df.columns and 'finish_cv_last5' in df.columns:
            df['gap_ability_stability'] = df['gap_ability_popularity'] * (1 / (df['finish_cv_last5'] + 0.1))
        
        # 【新規】オッズ期待値指標
        if 'popularity' in df.columns and 'past_5_finish_position_mean' in df.columns:
            # 人気順と実力順の差が大きいほど「割安」
            df['value_score'] = df['popularity'] - df['ability_rank']
        
        # 【新規】騎手×コース適性
        if 'jockey_win_rate' in df.columns and 'sire_course_win_rate' in df.columns:
            df['jockey_course_synergy'] = df['jockey_win_rate'] * df['sire_course_win_rate']
        
        logging.info("交互作用特徴量追加完了")
        return df
    ...
    def prepare_features(self, df: pd.DataFrame) -> list:
        ...
        new_features = [
            ...
            'gap_form_popularity',
            'gap_ability_stability',
            'value_score',
            'jockey_course_synergy',
        ]
    ...
    def prepare_target(self, df: pd.DataFrame):
        """ターゲット変数を準備（v3.3と同様だが微調整）"""
        logging.info("ターゲット変数を準備中...")
    ...
    def train(self, df: pd.DataFrame, feature_cols: list, n_trials: int = 50):
        """モデル学習"""
        logging.info("=" * 60)
        logging.info("μモデル v3.7 学習開始（安定性重視版）")
        logging.info("=" * 60)
        ...
        logging.info("=" * 60)
        logging.info(f"【最終結果: μ v3.7（安定性重視版）】")
