...
脚質×展開特徴量を追加したV31モデルを8年間Walk-forward検証
"""
import pandas as pd
import numpy as np
...
def train_v31(train_df, valid_df, feature_cols):
    """V31モデル学習（強化正則化）"""
    params = {
        'objective': 'binary',
        'metric': 'auc',
...
def main():
    ...
    for year, train_end, valid_start, valid_end, test_start, test_end in tqdm(test_periods, desc="Walk-forward検証"):
        ...
        # 特徴量生成（V31）
        print("  V31特徴量生成中...")
        engine = LeakFreeFeatureEngineerV31()
        ...
        # モデル学習
        print("  V31学習中...")
        model = train_v31(train_f, valid_f, feature_cols)
    ...
    results_df.to_csv(output_dir / "v31_evaluation.csv", index=False)
    print(f"\n結果を {output_dir / 'v31_evaluation.csv'} に保存しました。")
