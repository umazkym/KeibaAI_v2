#!/usr/bin/env python3
"""
μモデル v5.3 学習スクリプト（正則化緩和版）

【目標】v3.5超え（Test ROI > 81.84%）

【変更点】
1. 正則化緩和
   - min_child_samples: 20-60（30-150から緩和）
   - num_leaves: 30-100（25-80から緩和）
2. 特徴量追加
   - horse_best_nr: 過去最高NR（ポテンシャル上限）
"""
...
class MuV53Trainer:
    ...
    def __init__(self):
        ...
        self.output_dir = Path('keibaai/models/mu_v5_3')
    ...
    def get_distance_category(distance_m):
        for cat, (low, high) in MuV53Trainer.DISTANCE_CATEGORIES.items():
    ...
    def generate_horse_features(self, df, races_df):
        """馬特徴量生成（horse_best_nr追加）"""
        logging.info("=" * 60)
        logging.info("v5.3: 馬特徴量生成（正則化緩和版）")
        logging.info("=" * 60)
    ...
    def train(self, df, feature_cols, n_trials=50):
        logging.info("=" * 60)
        logging.info("μモデル v5.3 学習開始（正則化緩和版）")
        logging.info("=" * 60)
        ...
        logging.info("=" * 60)
        logging.info("【最終結果: μ v5.3（正則化緩和版）】")
        logging.info("=" * 60)
    ...
    def save_model(self, features, valid_roi, test_roi, best_params):
        with open(self.output_dir / 'mu_v5_3_ranker.pkl', 'wb') as f:
        ...
        metadata = {
            'version': 'v5.3', 'description': '正則化緩和版+horse_best_nr',
