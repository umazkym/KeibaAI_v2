#!/usr/bin/env python3
"""
μv2.7 モデル学習スクリプト (train_mu_v2_7.py)

【変更点 from v2.6】
- ROI最大化戦略 Phase 3: 新特徴量の導入 (ROI Maximization Features)
- 7つの新特徴量 (Time/Pace, Bias, Pedigree, Field, Market) を使用。
- オッズ加重学習 (Odds-Weighted Training) は継続。
- 保存先ディレクトリを `keibaai/models/mu_v2_7` に変更。
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('train_mu_v2_7.log')
    ]
)

class MuV27ROITrainer:
    def __init__(self, data_dir: str = 'keibaai/data/parsed/parquet'):
        self.data_dir = Path(data_dir)
        # v2.7用のディレクトリ
        self.models_dir = Path('keibaai/models/mu_v2_7')
    ...
    def generate_rolling_features(self, df: pd.DataFrame, ped_df: pd.DataFrame, prof_df: pd.DataFrame, dry_run: bool = False) -> pd.DataFrame:
        ...
        """
        logging.info("=" * 50)
        logging.info("ローリング特徴量生成を開始します (v2.7 ROI Maximization Mode)")
        logging.info("=" * 50)
        ...
        # 保存 (v2.7用のファイル名)
        if not dry_run:
            save_path = self.models_dir / 'train_data_mu_v2_7.parquet'
    ...
    def train_model(self, df: pd.DataFrame, n_trials: int = 50):
        ...
        logging.info("=" * 50)
        logging.info("モデル学習を開始します (v2.7 Weighted Training)")
        logging.info("=" * 50)
        ...
        # モデル保存
        model_path = self.models_dir / 'mu_v2_7_model.txt'
        model.save_model(str(model_path))
        
        with open(self.models_dir / 'mu_v2_7_model.pkl', 'wb') as f:
            pickle.dump(model, f)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Train μ v2.7 model with ROI Features & Odds-Weighted Training')
    ...
    trainer = MuV27ROITrainer()
    
    # 1. データ読み込み
    df, ped_df, prof_df = trainer.load_data()
    
    # 2. 特徴量生成
    # v2.7用のデータファイルが存在するか確認
    train_data_path = trainer.models_dir / 'train_data_mu_v2_7.parquet'
