...
def main():
    ...
    logger.info("=" * 70)
    logger.info("V6特徴量エンジニアテスト（厳密時点考慮版）")
    logger.info("=" * 70)
    ...
    # V6
    from keibaai.src.features.leak_free_feature_engineer_v6 import LeakFreeFeatureEngineerV6
    
    logger.info("\nV6 fit...")
    fe = LeakFreeFeatureEngineerV6()
    fe.fit(
        races_df=train_df,
    ...
    if train_rate - test_rate < 5:
        logger.info("✓ リークなし！差分 < 5%")
    elif train_rate - test_rate < 10:
        logger.info("✓ 軽微な過学習、差分 < 10%")
    else:
