#!/usr/bin/env python3
"""
μモデル v4.1 学習スクリプト（攻めと整理版）

【改善点】
1. 種牡馬コンテキスト特徴量追加: sire_turf_dirt_diff, sire_wet_diff, sire_left_right_diff
2. 旧通算特徴量削除: jockey_win_rate, sire_win_rate等を削除して上位互換のコンテキスト特徴量を強制使用
3. スムージングC=20に緩和（シグナル復元）

【設計思想】
- 血統は市場の非効率性がある→ROI向上の鍵
- 旧特徴量削除でモデルに「コンテキスト別」を強制使用させる
"""
...
class MuV41Trainer:
    ...
    # ベイズスムージング定数（緩和）
    SMOOTHING_C_JOCKEY = 20  # 騎手用
    SMOOTHING_C_SIRE = 30    # 種牡馬用（分散しやすいので強め維持）
    
    # 削除する旧通算特徴量（上位互換があるため不要）
    REDUNDANT_FEATURES = [
        'jockey_win_rate',        # → jockey_{context}_ae で代替
        'jockey_place_rate',      # → 複勝率も不要
        'sire_win_rate',          # → sire_turf_dirt_diff等で代替
        'sire_avg_finish',        # → コンテキスト別NRで代替
    ]
    
    def __init__(self):
        self.base_model_dir = Path('keibaai/models/mu_v3_3')
        self.output_dir = Path('keibaai/models/mu_v4_1')
    ...
    def get_turn_direction(venue: str) -> str:
        if venue in MuV41Trainer.LEFT_TURN_VENUES:
            return 'left'
        return 'right'
    
    @staticmethod
    def get_slope_type(venue: str) -> str:
        if venue in MuV41Trainer.STEEP_SLOPE_VENUES:
            return 'steep'
        return 'flat'
    ...
    def load_data(self):
        ...
        if pedigree_df is not None:
            logging.info(f"  血統データ: {len(pedigree_df):,} rows, columns: {pedigree_df.columns.tolist()[:5]}")
    ...
    def generate_context_aware_features(self, df: pd.DataFrame, perf_df: pd.DataFrame,
        ...
        logging.info("=" * 60)
        logging.info("v4.1 Context-Aware特徴量を生成中（攻めと整理版）...")
        logging.info(f"  騎手スムージング C = {self.SMOOTHING_C_JOCKEY}")
        logging.info(f"  種牡馬スムージング C = {self.SMOOTHING_C_SIRE}")
        logging.info("=" * 60)
        ...
        if pedigree_df is not None and 'generation' in pedigree_df.columns:
            ...
            # 道悪NR
            wet_sire_data = perf_with_sire[perf_with_sire['is_wet']]
            wet_sire_stats = wet_sire_data.groupby('sire_id').agg({
                'normalized_rank': 'mean', 'race_id': 'count'
            }).reset_index()
            wet_sire_stats.columns = ['sire_id', 'sire_wet_nr', 'sire_wet_count']
            
            # 道悪-良馬場差分
            cond_stats = good_stats.merge(wet_sire_stats, on='sire_id', how='outer').fillna(0)
            ...
            logging.info("    sire_turf_dirt_diff, sire_wet_diff, sire_left_right_diff 生成完了")
    ...
    def prepare_features(self, df: pd.DataFrame) -> list:
        ...
        logging.info(f"削除した旧特徴量: {self.REDUNDANT_FEATURES}")
    ...
    def train(self, df: pd.DataFrame, feature_cols: list, n_trials: int = 30):
        logging.info("=" * 60)
        logging.info("μモデル v4.1 学習開始（攻めと整理版）")
        logging.info("=" * 60)
        ...
        def objective(trial):
            params = {
                ...
                'feature_fraction': trial.suggest_float('feature_fraction', 0.45, 0.8),
            ...
            # Valid-Test差のペナルティを少し緩和
            gap_penalty = abs(valid_roi - test_roi) * 0.3
        ...
        logging.info("=" * 60)
        logging.info(f"【最終結果: μ v4.1（攻めと整理版）】")
        logging.info("=" * 60)
    ...
    def save_model(self, features, valid_roi, test_roi, best_params):
        with open(self.output_dir / 'mu_v4_1_ranker.pkl', 'wb') as f:
        ...
        metadata = {
            'version': 'v4.1',
            'description': '攻めと整理版（種牡馬追加+旧特徴量削除+C=20緩和）',
            'valid_roi': valid_roi,
            'test_roi': test_roi,
            ...
            'removed_features': self.REDUNDANT_FEATURES,
