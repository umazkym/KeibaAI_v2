...
log_file = log_dir / f"train_mu_time_v4_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def save_model(model, feature_cols, metrics, base_time_stats):
    """モデル保存"""
    output_dir = project_root / "keibaai" / "models" / "mu_time_v4_1"
...
def main():
    logger.info("=" * 60)
    logger.info("μ_time_v4.1 組織化緩和実験版 訓練スクリプト開始")
    logger.info("=" * 60)
