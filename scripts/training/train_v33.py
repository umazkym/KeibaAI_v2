...
脚質×ペース適合度特徴量を追加したV33モデルを8年間Walk-forward検証
"""
import pandas as pd
import numpy as np
...
def train_v33(train_df, valid_df, feature_cols):
    params = {
        'objective': 'binary',
        'metric': 'auc',
        'verbosity': -1,
...
def analyze_predictions(test_df, preds):
    ...
    if 'popularity' in bet_df.columns:
        ...
        # 穴馬分析（4番人気以下）
        dark_horse_bets = bet_df[bet_df['popularity'] >= 4]
        dark_horse_hits = dark_horse_bets[dark_horse_bets['is_hit']]
        dark_horse_roi = dark_horse_hits['win_odds'].sum() / len(dark_horse_bets) * 100 if len(dark_horse_bets) > 0 else 0
        dark_horse_rate = len(dark_horse_bets) / len(bet_df) * 100 if len(bet_df) > 0 else 0
    else:
        pop1_roi, non_pop1_roi, pop1_rate = 0, 0, 0
        dark_horse_roi, dark_horse_rate = 0, 0
    ...
    return {
        ...
        'dark_horse_rate': dark_horse_rate,
        'dark_horse_roi': dark_horse_roi,
    }
...
def main():
    ...
    for year, train_end, valid_start, valid_end, test_start, test_end in tqdm(test_periods, desc="Walk-forward検証"):
        ...
        print("  V33学習中...")
        model = train_v33(train_f, valid_f, feature_cols)
        ...
        print(f"  【重要】非1番人気ROI: {result['non_pop1_roi']:.1f}%, 穴馬(4人気以下)ROI: {result['dark_horse_roi']:.1f}%")
    ...
    print("\n【年別ROI推移】(※非1番人気ROIが真の実力)")
    print("-" * 80)
    print(f"{'年':<6} {'ROI':>8} {'非1人気ROI':>12} {'穴馬ROI':>10} {'1人気率':>10}")
    print("-" * 80)
    for _, row in results_df.iterrows():
        print(f"{row['year']:<6} {row['roi']:>7.1f}% {row['non_pop1_roi']:>11.1f}% {row['dark_horse_roi']:>9.1f}% {row['pop1_rate']:>9.1f}%")
    
    print("-" * 80)
    print(f"{'平均':<6} {results_df['roi'].mean():>7.1f}% {results_df['non_pop1_roi'].mean():>11.1f}% {results_df['dark_horse_roi'].mean():>9.1f}% {results_df['pop1_rate'].mean():>9.1f}%")
    print(f"{'σ':<6} {results_df['roi'].std():>7.1f}%")
    
    print(f"\n【指標】")
    print(f"  8年平均ROI: {results_df['roi'].mean():.1f}%")
    print(f"  ROI σ: {results_df['roi'].std():.1f}%")
    print(f"  【重要】非1番人気選択時ROI平均: {results_df['non_pop1_roi'].mean():.1f}%")
    print(f"  【重要】穴馬(4人気以下)選択時ROI平均: {results_df['dark_horse_roi'].mean():.1f}%")
    
    output_dir = project_root / "outputs" / "analysis"
    output_dir.mkdir(parents=True, exist_ok=True)
    results_df.to_csv(output_dir / "v33_evaluation.csv", index=False)
    print(f"\n結果を {output_dir / 'v33_evaluation.csv'} に保存しました。")
