#!/usr/bin/env python3
"""
μモデル v11.2 学習スクリプト（穴馬発掘強化版 + EV閾値最適化）

【v11.0からの改善点】
1. Optunaトライアル数増加（100回）
2. EV閾値最適化でベット判断を改善
3. バランスの取れた正則化パラメータ
...
class MuV112Trainer:
    """μモデル v11.2 穴馬発掘強化版 + EV閾値最適化"""
    
    # 過学習防止のための正則化パラメータ範囲
    REGULARIZATION_RANGES = {
        'num_leaves': (20, 50),  # バランス
        'lambda_l1': (5.0, 25.0),  # やや高め
        'lambda_l2': (3.0, 15.0),  # やや高め
        'min_child_samples': (80, 180),  # やや高め
        'learning_rate': (0.01, 0.04),  # 標準
        'feature_fraction': (0.4, 0.65),  # やや低め
    ...
    def __init__(self, train_cutoff: str = '2023-01-01'):
        ...
        self.output_dir = Path('keibaai/models/mu_v11_2')
    ...
    def save_model(self, features, valid_roi, test_roi, gap, best_params, 
                   valid_fav1_rate, test_fav1_rate):
        """モデル保存"""
        with open(self.output_dir / 'mu_v11_2_ranker.pkl', 'wb') as f:
        ...
        metadata = {
            'version': 'v11.2',
            'description': '穴馬発掘強化版 + EV閾値最適化（100トライアル学習）',
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='μモデル v11.2 学習（EV閾値最適化）')
    parser.add_argument('--trials', type=int, default=100, help='Optunaトライアル数')
    args = parser.parse_args()
    
    trainer = MuV112Trainer()
