...
def main():
    ...
    logger.info("=" * 70)
    logger.info("V5特徴量エンジニアテスト（正則化強化版）")
    logger.info("=" * 70)
    ...
    valid_start = test_start - pd.DateOffset(months=3)
    train_end = valid_start - pd.DateOffset(months=1)
    
    train_df = races_df[races_df['race_date'] < train_end].copy()
    valid_df = races_df[(races_df['race_date'] >= valid_start) & (races_df['race_date'] < test_start)].copy()
    ...
    valid_features['score'] = model.predict(valid_features[feature_cols].fillna(0))
    valid_features['rank'] = valid_features.groupby('race_id')['score'].rank(ascending=False)
    valid_top1 = valid_features[valid_features['rank'] == 1]
    ...
    # 的中率
    train_rate = (train_top1['finish_position'] == 1).mean() * 100
    valid_rate = (valid_top1['finish_position'] == 1).mean() * 100
    test_rate = (test_top1['finish_position'] == 1).mean() * 100
    
    logger.info("\n" + "=" * 70)
    logger.info("リークチェック: Train vs Valid vs Test的中率")
    logger.info("=" * 70)
    logger.info(f"Train的中率: {train_rate:.1f}%")
    logger.info(f"Valid的中率: {valid_rate:.1f}%")
    logger.info(f"Test的中率:  {test_rate:.1f}%")
    logger.info(f"Train-Test差分: {train_rate - test_rate:.1f}%")
    logger.info(f"Valid-Test差分: {valid_rate - test_rate:.1f}%")
    
    if train_rate - test_rate < 15:
        logger.info("✓ 過学習軽減成功！")
    elif train_rate - test_rate < 30:
        logger.info("△ 軽度の過学習")
    else:
        logger.warning("✗ まだ過学習傾向あり")
