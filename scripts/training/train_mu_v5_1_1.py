#!/usr/bin/env python3
"""
μモデル v5.1.1 学習スクリプト（データパイプライン修復版）

【修正点】race_id依存を排除、(horse_id, date)でマージ
- NaN率20-30%以下を目標
- カバレッジ確保によりリーク修正版の真価を発揮
"""
...
class MuV511Trainer:
    """μモデル v5.1.1 学習クラス（データパイプライン修復版）"""
    ...
    def __init__(self):
        ...
        self.output_dir = Path('keibaai/models/mu_v5_1_1')
    ...
    def get_distance_category(distance_m):
        for cat, (low, high) in MuV511Trainer.DISTANCE_CATEGORIES.items():
    ...
    def generate_horse_features(self, df, perf_df):
        """馬特徴量生成（horse_id + dateでマージ）"""
        logging.info("=" * 60)
        logging.info("v5.1.1: 馬特徴量生成（データパイプライン修復版）")
        logging.info("=" * 60)
        ...
        perf['n_runners'] = pd.to_numeric(perf['head_count'], errors='coerce').fillna(16)
        ...
        logging.info("  特徴量生成（expanding + shift(1)）...")
        ...
        # ========================================
        # train_dataとマージ（horse_id + race_date）
        # ========================================
        ...
        for col in horse_features:
            if col in result.columns:
                ...
                logging.info(f"    {col}: NaN率 {nan_rate:.1%}")
    ...
    def save_model(self, features, valid_roi, test_roi, best_params):
        with open(self.output_dir / 'mu_v5_1_1_ranker.pkl', 'wb') as f:
        ...
        metadata = {
            'version': 'v5.1.1', 'description': 'データパイプライン修復版',
