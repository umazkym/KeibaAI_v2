...
def load_data_for_features(
    ...
    parsed_base_dir = Path(paths_config.get('parsed_data_path', 'data/parsed')) / 'parquet'
    ...
    if shutuba_df.empty:
        logging.error(f"期間 {start_dt} - {end_dt} の出馬表データが見つかりません。処理を中止します。")
        return {}
...
def main():
    ...
    # --- 0. 設定とロギング ---
    # (注: load_config と setup_logging は run_scraping_pipeline_local.py と同じ実装を想定)
    # (ここでは簡易的に設定をロード)
    try:
        with open(args.config, 'r') as f:
            default_config = yaml.safe_load(f)
        paths_config = default_config
    ...
    if features_df.empty:
        logging.error("特徴量生成の結果が空です。")
        sys.exit(1)

    # --- 5. 特徴量保存 ---
    output_dir = Path(paths_config.get('features_path', 'data/features')) / 'parquet'
