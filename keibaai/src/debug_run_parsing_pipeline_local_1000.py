# keibaai/src/debug_run_parsing_pipeline_local_1000.py
# (元ファイル: run_parsing_pipeline_local.py)
#
# デバッグ用に各パーサーの処理対象ファイル数を1000件に制限しています。
...
def main():
    ...
    log.info("データ整形パイプラインを開始します... (デバッグモード: 最大1000件/各処理)")
    ...
    try:
        ...
        # --- ▼▼▼ デバッグ：制限 ▼▼▼ ---
        if len(race_html_files) > DEBUG_FILE_LIMIT:
            race_html_files = race_html_files[:DEBUG_FILE_LIMIT]
            log.info(f"デバッグのため、処理対象を {len(race_html_files)} 件に制限します。")
        # --- ▲▲▲ デバッグ：制限 ▲▲▲ ---
        ...
        # --- ▼▼▼ デバッグ：制限 ▼▼▼ ---
        if len(shutuba_html_files) > DEBUG_FILE_LIMIT:
            shutuba_html_files = shutuba_html_files[:DEBUG_FILE_LIMIT]
            log.info(f"デバッグのため、処理対象を {len(shutuba_html_files)} 件に制限します。")
        # --- ▲▲▲ デバッグ：制限 ▲▲▲ ---
        ...
        # --- ▼▼▼ デバッグ：制限 ▼▼▼ ---
        if len(horse_html_files) > DEBUG_FILE_LIMIT:
            horse_html_files = horse_html_files[:DEBUG_FILE_LIMIT]
            log.info(f"デバッグのため、処理対象を {len(horse_html_files)} 件に制限します。")
        # --- ▲▲▲ デバッグ：制限 ▲▲▲ ---
        ...
        # --- ▼▼▼ デバッグ：制限 ▼▼▼ ---
        if len(ped_html_files) > DEBUG_FILE_LIMIT:
            ped_html_files = ped_html_files[:DEBUG_FILE_LIMIT]
            log.info(f"デバッグのため、処理対象を {len(ped_html_files)} 件に制限します。")
        # --- ▲▲▲ デバッグ：制限 ▲▲▲ ---
